...
server <- function(input, output, session) {
  EPoMAB <- EPoMAB_heurist # still dataset: isn't updated!
  # reactiveFileReader doesn't work, solution below...
  #EPoMAB_reactive <- reactiveFileReader(1000, session = NULL, "data/EPoMAB_heurist.rda", load, envir = .GlobalEnv)
  EPoMAB_reactive <- reactiveVal(value = NULL) # Create empty reactive value for reactivet dataset
  observe({ # Every 2 seconds load EPoMAB_heurist.rda (refresh data if changed)
    invalidateLater(2000, session)
    n <- new.env()
    print("load EPoMAB")
    env <- load("data/EPoMAB_heurist.rda", envir = n)
    EPoMAB_reactive(n[[names(n)]])
  })
  source("R/_server_FUN.R", local = TRUE)
  source("./heurist/heurist2epomab.R", local = TRUE)
  # Data
  ...
  output$table <- DT::renderDataTable({
    ...
    #               rownames = FALSE)
    # For Heurist dataset
    DT::datatable(dataset() %>% select(-c("Description","Bibliographie","Commentaires","Coordonnees")), 
                  rownames = FALSE)
    })
  ...
  datasetmap <- eventReactive(dataset(), {
    ...
    data <- data %>% 
      drop_na(Coordonnees) %>% 
      st_as_sf(wkt = "Coordonnees", crs = 4326) %>% 
  ...
  output$swordmap <- renderLeaflet({
    leaflet() %>% 
      ...
      addPolylines(data = rivers_europe, ## hidden rivers layer
                   ...
                   group = ~dissolve) %>%
      addCircles(data = datasetmap(), # used to be datasetmap()
  ...
  observeEvent(input$swordmap_shape_click, {
    if(input$swordmap_shape_click$group == "points") {
      ...
      proxy %>% showGroup(group = paste0(EPoMAB_reactive()$Commune[which(EPoMAB$ID == input$swordmap_shape_click$id)],
  ...
  maptable <- reactive({
    EPoMAB_reactive() %>% filter(ID %in% selected$ids) -> clickeddata
  ...
  output$clickedpoint.table <- DT::renderDataTable({
    DT::datatable(data = maptable()%>% select(-c("Description","Bibliographie","Commentaires","Coordonnees")),
                  rownames = FALSE,
                  options=list(dom = "tp")
                  )
  })
  ...
  # Refreshing dataset button
  observeEvent(input$refresh, {
    busy_text <- switch(input$lang,
                        "Fr" = "Rafraîchissement des données depuis la base Heurist en cours...",
                        "En" = "Refresh dataset from Heurist database in progress...",
                        stop("Option inconnue")
    )
    show_modal_spinner(text = busy_text) # show the modal window
    heurist2epomab() # Retrieve data from Heurist: update EPoMAB_heurist.rda
